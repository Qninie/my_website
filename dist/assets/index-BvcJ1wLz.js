  ...
  <main class="home-page">
          ...
          <div class="blossom-tree" aria-label="Interactive blossom memory tree">
            ...
            ${S.map((e,t)=>`
              <button class="memory-flower" data-index="${t}" style="--tree-x:${e.tree.x}%;--tree-y:${e.tree.y}%" aria-label="Discover ${e.title} memory">
                <span class="bud-layer">${L("memory-closed-flower")}</span>
                <span class="bloom-layer">${L("memory-flower-svg")}</span>
              </button>`).join("")}
    ...
    <section id="skills" class="skills-section" aria-labelledby="skills-title">
      ...
      <div class="skills-frame">
        ...
        <span class="frame-flower frame-flower--one" aria-hidden="true">${L("frame-flower-svg")}</span>
        <span class="frame-flower frame-flower--two" aria-hidden="true">${L("frame-flower-svg")}</span>
    ...
    <section id="contact" class="contact-section" aria-labelledby="contact-title">
      ...
      <div class="contact-layout">
        ...
        <div class="contact-form-wrap">
          <span class="contact-blossom" aria-hidden="true">${L("contact-flower-svg")}</span>
...
`;const D=[...document.querySelectorAll(".greeting-word")],be=document.querySelector(".begin-cue");let q=0;window.setInterval(()=>{D[q].classList.remove("is-visible"),q=(q+1)%D.length,D[q].classList.add("is-visible")},1450);const V=document.querySelector(".sakura-cursor"),we=matchMedia("(hover:hover) and (pointer:fine) and (min-width:1025px) and (prefers-reduced-motion:no-preference)").matches;let F=0,B={x:-80,y:-80},Q=0;const H=new Set;we&&window.addEventListener("pointermove",e=>{if(B={x:e.clientX,y:e.clientY},F||(F=requestAnimationFrame(()=>{V.style.transform=`translate3d(${B.x}px,${B.y}px,0)`,V.classList.add("is-visible"),F=0})),performance.now()-Q<110||H.size>=12)return;Q=performance.now();const t=document.createElement("i");t.className="trail-petal",t.style.cssText=`left:${e.clientX}px;top:${e.clientY}px;--drift:${(Math.random()-.5)*60}px;--spin:${(Math.random()-.5)*240}deg`,H.add(t),document.body.append(t),t.addEventListener("animationend",()=>{H.delete(t),t.remove()},{once:!0})},{passive:!0});const b=document.querySelector(".hero-scene");document.querySelector(".primary-button");const g=document.querySelector(".scroll-butterfly-guide"),Se=g.querySelector(".guide-tip");new IntersectionObserver(([e])=>{e.isIntersecting&&!b.classList.contains("has-grown")&&b.classList.add("has-grown")},{threshold:.28}).observe(b);const x=document.querySelector(".hero-nature"),G=new Set;let J=0;matchMedia("(hover:hover) and (pointer:fine) and (prefers-reduced-motion:no-preference)").matches&&(b.addEventListener("pointermove",e=>{const t=b.getBoundingClientRect(),s={x:t.left+t.width*.77,y:t.top+t.height*.34},a=e.clientX-s.x,i=e.clientY-s.y,r=Math.hypot(a,i);if(r>180){x.style.setProperty("--react-x","0deg"),x.style.setProperty("--react-y","0px");return}const n=1-r/180;if(x.style.setProperty("--react-x",`${Math.max(-3,Math.min(3,-a/55*n))}deg`),x.style.setProperty("--react-y",`${-i/20*n}px`),performance.now()-J>850&&G.size<3&&Math.random()>.55){J=performance.now();const o=document.createElement("i");o.className="released-petal",o.style.cssText=`left:${68+Math.random()*22}%;top:${24+Math.random()*28}%;--petal-drift:${(Math.random()-.5)*140}px;--petal-spin:${160+Math.random()*260}deg`,G.add(o),b.append(o),o.addEventListener("animationend",()=>{G.delete(o),o.remove()},{once:!0})}},{passive:!0}),b.addEventListener("pointerleave",()=>{x.style.setProperty("--react-x","0deg"),x.style.setProperty("--react-y","0px")}));const j=document.querySelector(".door-scene"),R=document.querySelector(".journey-section"),ke=document.querySelector(".timeline-rail"),Le=document.querySelector('.site-nav a[data-section="door-scene"]'),te=.68,W=matchMedia("(prefers-reduced-motion:reduce)").matches;let E=!1,_=0,K=scrollY;function se(){const e=Math.max(0,j.offsetHeight-innerHeight);return j.offsetTop+e*te}Le.addEventListener("click",e=>{e.preventDefault();const t=se();E=!0,history.replaceState(null,"","#door-scene"),window.scrollTo({top:t,behavior:W?"auto":"smooth"})});let U=0,Y=!1,ae=!1;function xe(){if(Y||W)return;const e=[...document.querySelectorAll(".memory-flower")].map(n=>{const o=n.getBoundingClientRect();return{x:o.left+o.width/2,y:o.top+o.height/2}});if(e.length!==S.length)return;Y=!0;const t={x:innerWidth*.78,y:innerHeight*.62},s=[{x:-60,y:Math.max(innerHeight*.55,e[0].y+70)},...e,t],a=[0,.12,.32,.5,.68,.86],i=s.map((n,o)=>({offset:a[o],transform:`translate3d(${n.x}px,${n.y}px,0) translate(-50%,-50%) scale(${o?1:.72})`,opacity:o?1:0,easing:"cubic-bezier(.35,.05,.25,1)"})),r=g.animate(i,{duration:7200,fill:"forwards"});r.finished.then(()=>{ae=!0,g.style.setProperty("--guide-screen-x","78vw"),g.style.setProperty("--guide-screen-y","62vh"),g.classList.add("has-tip"),r.cancel()}).catch(()=>{})}function ie(){const e=j.getBoundingClientRect(),t=j.offsetHeight-innerHeight,s=Math.max(1,t*te),a=Math.max(0,Math.min(1,-e.top/s)),i=scrollY>K;!W&&!E&&i&&_<.94&&a>=.94&&(E=!0,window.scrollTo({top:se(),behavior:"smooth"})),a<.65&&(E=!1),_=a,K=scrollY,j.style.setProperty("--door-progress",a.toFixed(3));const r=R.getBoundingClientRect(),n=ke.getBoundingClientRect(),o=Math.max(105,Math.min(innerHeight*.56,n.bottom-18)),u=Math.max(0,Math.min(1,(o-n.top)/Math.max(1,n.height)));R.style.setProperty("--journey-progress",u.toFixed(3));const p=a>.94&&r.top>innerHeight*.55,f=n.top<=innerHeight*.56&&n.bottom>=105,v=p||f;let k,I;p?(k=innerWidth*.78,I=innerHeight*.54):f?(k=n.left+n.width/2,I=o):(k=innerWidth*.5,I=innerHeight*.6),g.style.setProperty("--guide-screen-x",`${k}px`),g.style.setProperty("--guide-screen-y",`${I}px`),g.classList.toggle("is-following",v),g.classList.toggle("has-tip",p&&ae),g.classList.toggle("is-tree-orbiting",p),g.classList.toggle("is-on-timeline",f),g.setAttribute("aria-hidden",String(!v)),Se.textContent="Click a flower to discover more about Queenie.",p&&xe(),b.classList.toggle("guide-is-travelling",v),document.querySelector(".home-nav").classList.toggle("has-background",scrollY>8),U=0}window.addEventListener("scroll",()=>{U||(U=requestAnimationFrame(ie))},{passive:!0});ie();const M=[...document.querySelectorAll(".site-nav a")];M.map(e=>document.getElementById(e.dataset.section)).filter(Boolean);new IntersectionObserver(e=>{const t=e.filter(s=>s.isIntersecting).sort((s,a)=>a.intersectionRatio-s.intersectionRatio)[0];t&&M.forEach(s=>s.classList.toggle("is-active",s.dataset.section===t.target.id))},{rootMargin:"-34% 0px -52%",threshold:[0,.2,.5]}).observe(j);new IntersectionObserver(e=>{e[0].isIntersecting&&M.forEach(t=>t.classList.toggle("is-active",t.dataset.section==="journey"))},{rootMargin:"-32% 0px -55%",threshold:0}).observe(R);const $e=document.querySelector(".work-section");new IntersectionObserver(e=>{e[0].isIntersecting&&M.forEach(t=>t.classList.toggle("is-active",t.dataset.section==="work"))},{rootMargin:"-32% 0px -55%",threshold:0}).observe($e);const N=document.querySelector(".skills-section");new IntersectionObserver(e=>{e[0].isIntersecting&&M.forEach(t=>t.classList.toggle("is-active",t.dataset.section==="skills"))},{rootMargin:"-32% 0px -55%",threshold:0}).observe(N);const je=document.querySelector(".contact-section");new IntersectionObserver(e=>{e[0].isIntersecting&&M.forEach(t=>t.classList.toggle("is-active",t.dataset.section==="contact"))},{rootMargin:"-32% 0px -55%",threshold:0}).observe(je);const Me=document.querySelectorAll(".journey-milestone"),Ie=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&t.target.classList.add("is-visible")})},{threshold:.2});Me.forEach(e=>Ie.observe(e));const T=document.querySelectorAll(".work-card"),qe=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&t.target.classList.add("is-visible")})},{threshold:.15});T.forEach(e=>qe.observe(e));const re=[...document.querySelectorAll(".work-filter")],Ce=document.querySelector(".work-empty");function Ee(e){const t=[...T].filter(s=>e==="all"||s.dataset.category===e);T.forEach(s=>{const a=t.includes(s);s.hidden=!a,s.classList.remove("is-filter-featured","is-filter-even")}),e!=="all"&&(t[0]?.classList.add("is-filter-featured"),t.slice(1).forEach((s,a)=>s.classList.toggle("is-filter-even",a%2===0))),Ce.hidden=t.length>0,re.forEach(s=>{const a=s.dataset.filter===e;s.classList.toggle("is-active",a),s.setAttribute("aria-pressed",String(a))})}re.forEach(e=>e.addEventListener("click",()=>Ee(e.dataset.filter)));const z=new WeakSet;function oe(e){if(!e||z.has(e))return;const t=[...e.querySelectorAll("img")];if(t.length<2)return;const s=e.closest(".project-preview, .case-study-visual")??e;let a=0,i=null;const r=u=>{const h=t[a];h.classList.add("is-leaving"),h.classList.remove("is-active"),window.setTimeout(()=>h.classList.remove("is-leaving"),600),a=u%t.length,t[a].classList.add("is-active")},n=()=>{i||(e.classList.add("is-playing"),i=window.setInterval(()=>r(a+1),1350))},o=()=>{window.clearInterval(i),i=null,e.classList.remove("is-playing"),r(0)};s.addEventListener("pointerenter",n),s.addEventListener("pointerleave",o),z.add(e)}document.querySelectorAll(".project-screenshot-carousel").forEach(oe);new IntersectionObserver(([e],t)=>{e.isIntersecting&&(N.classList.add("is-visible"),t.disconnect())},{threshold:.18}).observe(N);const C=document.querySelector(".letter-button"),Te=document.querySelector(".letter-note");C.addEventListener("click",()=>{const e=C.getAttribute("aria-expanded")==="true";C.setAttribute("aria-expanded",String(!e)),Te.hidden=e,C.querySelector("i").textContent=e?"＋":"−"});document.querySelector(".contact-form").addEventListener("submit",e=>{e.preventDefault();const t=new FormData(e.currentTarget),s=t.get("name").trim(),a=t.get("email").trim(),i=t.get("message").trim(),r=encodeURIComponent(`Portfolio enquiry from ${s}`),n=encodeURIComponent(`${i}

From: ${s}
Email: ${a}`);window.location.href=`mailto:qthamshinee@gmail.com?subject=${r}&body=${n}`});const l=document.querySelector(".project-modal"),ne=l.querySelector(".project-modal-close");let le=null;function Ae(e,t){const s=ee[e];le=t,l.className=`project-modal project-modal--${s.slug}`,l.querySelector(".case-number").textContent=s.number,l.querySelector("#project-modal-title").textContent=s.title,l.querySelector(".case-role").textContent=s.role,l.querySelector(".case-summary").textContent=s.summary,l.querySelector(".case-challenge").textContent=s.challenge,l.querySelector(".case-approach").textContent=s.approach,l.querySelector(".case-outcome").textContent=s.outcome;const a=l.querySelector(".case-highlights");a.hidden=!s.highlights,a.innerHTML=s.highlights?.map(([p,f])=>`<span><strong>${p}</strong>${f}</span>`).join("")??"",l.querySelector(".case-tools").innerHTML=s.tools.map(p=>`<li>${p}</li>`).join("");const i=l.querySelector(".case-study-visual"),r=i.querySelector(".case-screenshot-carousel"),n=Array.isArray(s.images)&&s.images.length>0;r.hidden=!n,n&&r.dataset.project!==s.slug&&(r.dataset.project=s.slug,r.innerHTML=s.images.map((p,f)=>`<img class="${f===0?"is-active":""}" src="${m(p)}" alt="">`).join("")),i.classList.toggle("has-project-image",!!s.image||n),i.style.backgroundImage=s.image?`url("${m(s.image)}")`:"",n&&oe(r);const o=l.querySelector(".case-live-link");o.href=s.link,o.textContent=s.slug==="language"||s.slug==="apcs"?"Watch project walkthrough":"View live project";const u=l.querySelector(".case-prototype-link");u.hidden=!s.prototype,s.prototype&&(u.href=s.prototype);const h=l.querySelector(".case-manual-link");h.hidden=!s.manual,s.manual&&(h.href=m(s.manual)),l.hidden=!1,document.body.classList.add("modal-open"),requestAnimationFrame(()=>l.classList.add("is-open")),ne.focus()}function ce(){l.classList.remove("is-open"),document.body.classList.remove("modal-open"),setTimeout(()=>{l.hidden=!0,le?.focus()},450)}T.forEach((e,t)=>{e.querySelectorAll(".project-preview,.case-study-link").forEach(s=>s.addEventListener("click",()=>Ae(t,s)))});ne.addEventListener("click",ce);const X=[...document.querySelectorAll(".memory-flower")],Pe=document.querySelector(".discovery-progress span"),c=document.querySelector(".memory-modal"),O=document.querySelector(".memory-garden"),De=document.querySelector(".tree-instruction"),$=document.querySelector(".tree-guide-butterfly"),Fe=document.querySelector(".discovery-progress"),d={viewMode:"tree",currentMemory:null,activeFlowerIndex:0,discoveredMemories:new Set};let de=null,pe=!1;const y=c.querySelector(".memory-photo-marquee");let w=null;y.addEventListener("pointerdown",e=>{y.classList.contains("is-single-photo")||(w={pointerId:e.pointerId,startX:e.clientX,startScroll:y.scrollLeft},y.classList.add("is-dragging"),y.setPointerCapture(e.pointerId))});y.addEventListener("pointermove",e=>{!w||w.pointerId!==e.pointerId||(y.scrollLeft=w.startScroll-(e.clientX-w.startX))});const me=e=>{!w||w.pointerId!==e.pointerId||(w=null,y.classList.remove("is-dragging"))};y.addEventListener("pointerup",me);y.addEventListener("pointercancel",me);function A(){const e=d.discoveredMemories.size===S.length;if(X.forEach((t,s)=>{const a=d.discoveredMemories.has(s),i=!e&&s===d.activeFlowerIndex&&!a;t.className=`memory-flower ${a?"is-bloomed":i?"is-active":"is-closed"}`,t.setAttribute("aria-current",i?"step":"false"),t.setAttribute("aria-label",`${a?"Revisit":"Discover"} ${S[s].title} memory`)}),Pe.textContent=String(Math.min(d.discoveredMemories.size+(e?0:1),4)).padStart(2,"0"),Fe.classList.toggle("is-complete",e),De.textContent=e?"Four moments, one growing journey.":"Select the glowing flower to discover a memory.",$.classList.toggle("is-settled",e),!e&&pe){const t=S[d.activeFlowerIndex].tree;$.style.setProperty("--guide-x",`${t.x}%`),$.style.setProperty("--guide-y",`${t.y}%`),$.classList.remove("is-guiding"),requestAnimationFrame(()=>$.classList.add("is-guiding"))}}new IntersectionObserver(([e],t)=>{e.isIntersecting&&(pe=!0,document.querySelector(".door-sticky").classList.add("tree-is-revealing"),A(),t.disconnect())},{threshold:.55}).observe(document.querySelector(".door-sticky"));function Z(e,t,s=!1){const a=S[e];de=t,c.dataset.memory=a.id,d.currentMemory=e,d.viewMode="memory",s&&d.discoveredMemories.add(e),A(),c.querySelector("#modal-title").textContent=a.title,c.querySelector(".modal-copy .scene-label").textContent=`${a.chapter} · A personal chapter`,c.querySelector(".modal-description").textContent=a.description,c.querySelector(".modal-detail").textContent=a.detail;const i=c.querySelector(".modal-image");i.className=`modal-image memory-image-${a.art}`,i.style.backgroundImage=a.image?`url("${a.image}")`:"",i.setAttribute("aria-label",`${a.title} placeholder image`);const r=c.querySelector(".memory-photo-marquee"),n=c.querySelector(".detail-blossom"),o=Array.isArray(a.gallery)?a.gallery.filter(h=>h.image):[],u=o.length>0;if(r.hidden=!u,r.classList.remove("is-single-photo"),n.hidden=u,c.querySelector(".memory-detail").classList.toggle("has-photo-gallery",u),u){const h=3.4285714285714284,p=20/7;r.style.setProperty("--memory-scroll-duration",`${o.length*h}s`),r.style.setProperty("--memory-scroll-duration-mobile",`${o.length*p}s`);const f=[...o,...o];r.querySelector(".memory-photo-track").innerHTML=f.map((v,k)=>`
      <figure class="memory-photo-card" ${k>=o.length?'aria-hidden="true"':""}>
